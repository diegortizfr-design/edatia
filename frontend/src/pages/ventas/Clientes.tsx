...
import { Plus, Search, Users, Eye, Pencil } from 'lucide-react'
                ...
                {(clientes as any[]).map((c: any) => (
                  <tr key={c.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      {c.nombreComercial && <p className="text-[10px] text-slate-400 uppercase font-bold">{c.nombreComercial}</p>}
                    </td>
                    <td className="px-6 py-4 text-slate-600 text-xs font-mono">
                      {c.tipoDocumento} {c.numeroDocumento}
                      {c.digitoVerificacion ? `-${c.digitoVerificacion}` : ''}
                    </td>
                    <td className="px-6 py-4 text-slate-500 text-xs">{c.municipio || '—'}</td>
